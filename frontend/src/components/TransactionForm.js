...
const [maturityDatePassed, setMaturityDatePassed] = useState(false);
  ...
  try {
    ...
    // Check if maturity date is passed
    if (maturityDateISO && maturityDateISO < today) {
      setMaturityDatePassed(true);
    } else {
      setMaturityDatePassed(false);
    }
  ...
  const handleChange = (e) => {
    ...
    setTransaction(prev => {
      ...
      if ((name === 'receivedAmount' || name === 'todayAmount') &&
        updated.todayAmount && updated.receivedAmount && !isNaN(updated.todayAmount) &&
        !isNaN(updated.receivedAmount) && Number(updated.todayAmount) !== 0) {
  ...
  const handleSubmit = () => {
    ...
    api.post('/transactions', {
      ...
      .then(() => {
        ...
        setSavedData(transaction);
        setDialogOpen(true);
        fetchNextEID(transaction.date);
        setTransaction(prev => ({
          regId: '', cusId: '', name: '', number: '', city: '',
          PID: '', ChitID: '', date: prev.date, todayAmount: prev.todayAmount,
          receivedAmount: '', goldGram: '', payMode: '', status: 'Received', EID: prev.EID,maturityDate: '',
        }));
      })
      .catch(() => {
